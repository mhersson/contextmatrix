...
func TestManager_DeleteSession_ClearsPendingToolUseID(t *testing.T) {
	store, err := sqlite.Open(filepath.Join(t.TempDir(), "chats.db"))
	require.NoError(t, err)
	t.Cleanup(func() { _ = store.Close() })
	...
	runner := &stubRunner{
		streamLogsFn: func(ctx context.Context, _ string, onEntry func(chat.LogEntry)) error {
			onEntry(chat.LogEntry{Type: "user_question", Content: "Q?", ToolUseID: "toolu_del"})
			close(delivered)
	...
	// After delete, the pending id must be gone.
...
func TestManager_DoClearContext_DropsPendingToolUseID(t *testing.T) {
	store, err := sqlite.Open(filepath.Join(t.TempDir(), "chats.db"))
	require.NoError(t, err)
	t.Cleanup(func() { _ = store.Close() })
	...
	runner := &stubRunner{
		streamLogsFn: func(ctx context.Context, _ string, onEntry func(chat.LogEntry)) error {
			onEntry(chat.LogEntry{Type: "user_question", Content: "Q?", ToolUseID: "toolu_clear"})
			close(delivered)
...
func TestManager_SendUserMessage_ToolUseID_SessionIsolation(t *testing.T) {
	...
	// Create two sessions before building the manager so we know the IDs.
	sessA, err := chat.NewManager(chat.Config{
		Store:   store,
		Runner:  &stubRunner{},
		Clock:   clock.Real(),
		IdleTTL: time.Hour,
	}).CreateSession(ctx, chat.CreateInput{Title: "A", CreatedBy: "human:test"})
	require.NoError(t, err)

	sessB, err := chat.NewManager(chat.Config{
		Store:   store,
		Runner:  &stubRunner{},
		Clock:   clock.Real(),
		IdleTTL: time.Hour,
	}).CreateSession(ctx, chat.CreateInput{Title: "B", CreatedBy: "human:test"})
	require.NoError(t, err)
	...
	runner := &stubRunner{
		streamLogsFn: func(ctx context.Context, sessionID string, onEntry func(chat.LogEntry)) error {
			switch sessionID {
			case sessA.ID:
				onEntry(chat.LogEntry{Type: "user_question", Content: "Q?", ToolUseID: "toolu_A"})
				close(readyA)
			case sessB.ID:
				onEntry(chat.LogEntry{Type: "user_question", Content: "Q?", ToolUseID: "toolu_B"})
				close(readyB)
			}
	...
	_, err = mgr.OpenSession(ctx, sessA.ID)
	require.NoError(t, err)

	t.Cleanup(func() { _ = mgr.EndSession(context.Background(), sessA.ID) })

	_, err = mgr.OpenSession(ctx, sessB.ID)
	require.NoError(t, err)

	t.Cleanup(func() { _ = mgr.EndSession(context.Background(), sessB.ID) })

	select {
	case <-readyA:
	case <-time.After(2 * time.Second):
		t.Fatal("session A: StreamLogs onEntry never invoked")
	}
