...
func TestKnowledge_WriteAndReadDoc(t *testing.T) {
	dir := t.TempDir()
	setupTestProject(t, dir, "p", "P")

	store, err := NewFilesystemStore(dir)
	require.NoError(t, err)

	require.NoError(t, store.WriteKnowledgeDoc(context.Background(), "p", "core", "architecture.md", []byte("# Arch\n")))

	got, err := store.ReadKnowledgeDoc(context.Background(), "p", "core", "architecture.md")
	require.NoError(t, err)
	assert.Equal(t, "# Arch\n", string(got))

	assert.FileExists(t, filepath.Join(dir, "p", "knowledge", "core", "architecture.md"))
}

func TestKnowledge_ReadDoc_NotFound(t *testing.T) {
	dir := t.TempDir()
	setupTestProject(t, dir, "p", "P")

	store, _ := NewFilesystemStore(dir)

	_, err := store.ReadKnowledgeDoc(context.Background(), "p", "core", "architecture.md")
	assert.ErrorIs(t, err, ErrKnowledgeDocNotFound)
}

func TestKnowledge_MetaRoundTrip(t *testing.T) {
	dir := t.TempDir()
	setupTestProject(t, dir, "p", "P")
	...
	meta := &board.KnowledgeMeta{
		SchemaVersion: 1,
		Repos: map[string]board.KnowledgeRepoMeta{
			"core": {URL: "git@github.com:o/core.git", LastBuiltCommit: "abc"},
		},
	}
	require.NoError(t, store.WriteKnowledgeMeta(context.Background(), "p", meta))

	loaded, err := store.ReadKnowledgeMeta(context.Background(), "p")
	require.NoError(t, err)
	assert.Equal(t, "abc", loaded.Repos["core"].LastBuiltCommit)
}

func TestKnowledge_MetaNotFoundReturnsEmpty(t *testing.T) {
	dir := t.TempDir()
	setupTestProject(t, dir, "p", "P")
...
func TestKnowledge_RejectsInvalidDocName(t *testing.T) {
	dir := t.TempDir()
	setupTestProject(t, dir, "p", "P")

	store, _ := NewFilesystemStore(dir)

	err := store.WriteKnowledgeDoc(context.Background(), "p", "core", "../escape", []byte("x"))
	assert.ErrorIs(t, err, ErrInvalidKnowledgeDoc)
}

func TestKnowledge_ListReposEmpty(t *testing.T) {
	dir := t.TempDir()
	setupTestProject(t, dir, "p", "P")

	store, _ := NewFilesystemStore(dir)

	repos, err := store.ListKnowledgeRepos(context.Background(), "p")
	require.NoError(t, err)
	assert.Empty(t, repos)
}

func TestKnowledge_ListReposAfterWrite(t *testing.T) {
	dir := t.TempDir()
	setupTestProject(t, dir, "p", "P")

	store, _ := NewFilesystemStore(dir)

	require.NoError(t, store.WriteKnowledgeDoc(context.Background(), "p", "core", "architecture.md", []byte("x")))
	require.NoError(t, store.WriteKnowledgeDoc(context.Background(), "p", "runner", "architecture.md", []byte("y")))

	repos, err := store.ListKnowledgeRepos(context.Background(), "p")
	require.NoError(t, err)
	assert.ElementsMatch(t, []string{"core", "runner"}, repos)
}

func TestKnowledge_DocExists(t *testing.T) {
	dir := t.TempDir()
	setupTestProject(t, dir, "p", "P")

	store, _ := NewFilesystemStore(dir)

	exists, err := store.KnowledgeDocExists(context.Background(), "p", "core", "architecture.md")
	require.NoError(t, err)
...
func TestFilesystemStore_ReadKnowledgeDoc_RejectsSymlink(t *testing.T) {
	t.Helper()
	dir := t.TempDir()
	setupTestProject(t, dir, "p", "P")
	...
	repoDir := filepath.Join(dir, "p", "knowledge", "core")
	require.NoError(t, os.MkdirAll(repoDir, 0o755))

	target := filepath.Join(dir, "secret.txt")
	require.NoError(t, os.WriteFile(target, []byte("hunter2"), 0o600))
	require.NoError(t, os.Symlink(target, filepath.Join(repoDir, "architecture.md")))

	_, err = store.ReadKnowledgeDoc(context.Background(), "p", "core", "architecture.md")
	require.Error(t, err)
	assert.ErrorIs(t, err, ErrKnowledgeDocSymlink)
}
