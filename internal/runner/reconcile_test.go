...
// TestReconciliationSweep_SkipsKBRefreshContainer confirms the sweep does
// NOT try to kill a knowledge-base refresh container by its synthetic
// "kb-refresh:<repo>" card_id. KB refresh has its own staleness janitor
// (internal/refresh) and the runner's container_timeout as the last-resort
// cap. The runner's /end-session and /kill validators reject non-card-shaped
// IDs, so attempting a kill here would fail with HTTP 400 — defending
// against this also avoids log noise from the failed webhooks.
func TestReconciliationSweep_SkipsKBRefreshContainer(t *testing.T) {
	ctx := t.Context()

	// Card-getter has NO entry for the synthetic id — the OLD sweep would
	// have treated this as a no-such-card kill. The new sweep skips it.
	cg := &fakeCardGetter{cards: map[string]*board.Card{}}
	fc := &fakeClient{
		listResult: []runner.ContainerInfo{
			{
				ContainerID: "f8b21f1c228d",
				CardID:      "kb-refresh:homelander",
				Project:     "homelander",
				State:       "running",
				StartedAt:   time.Now().Add(-5 * time.Minute),
			},
	...
	assert.Empty(t, fc.KillCalls(),
		"sweep must skip kb-refresh:* containers (managed by internal/refresh)")
}

// TestReconciliationSweep_KBRefreshSkipsAgeCap is the regression guard for
// the ordering bug: the age-cap rule used to fire BEFORE the kb-refresh
// prefix skip, so a long-running refresh past ContainerMaxAge would trigger
// a kill the runner rejects with HTTP 400 (synthetic id isn't card-shaped),
// producing per-tick log spam. Refresh staleness is owned by the
// internal/refresh registry janitor; the sweep must pass these through
// regardless of age.
func TestReconciliationSweep_KBRefreshSkipsAgeCap(t *testing.T) {
	ctx := t.Context()
	...
	time.Sleep(150 * time.Millisecond)
	assert.Empty(t, fc.KillCalls(),
		"sweep must skip kb-refresh:* containers regardless of age — refresh registry janitor handles staleness")
}
