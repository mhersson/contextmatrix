...
| File | Role |
|---|---|
| `web/src/hooks/useRingBuffer.ts` | Fixed-capacity circular buffer hook. `useRingBuffer(maxEntries)` returns `{ logs: readonly LogEntry[], append(entries), clear() }`. Internally keeps a pre-allocated array and a `head` index; each `append` writes at `head` and advances it modulo capacity — O(1) per entry, no full-array copy. React re-renders are triggered by a small integer counter (`setRenderCount`) rather than by replacing the log array on every event. The `logs` snapshot is rebuilt in insertion order on each render from the ref state. |
