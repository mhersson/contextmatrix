...
export function buildCardPatch(edited: Card, original: Card): PatchCardInput {
  ...
  if (!arraysEqual(edited.mob_phases, original.mob_phases)) {
    updates.mob_phases = edited.mob_phases ?? [];
  }
  if (!arraysEqual(edited.mob_guests, original.mob_guests)) {
    updates.mob_guests = edited.mob_guests ?? [];
  }
