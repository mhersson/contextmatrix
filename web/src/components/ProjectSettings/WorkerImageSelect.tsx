 ...
 *
 * A saved value missing from the fetched list stays selectable with a
 * warning so saving unrelated settings never breaks and nothing is silently
 * substituted. When the list cannot be loaded (backend down or disabled)
 * the options reduce to the backend default plus the saved value.
 */
export function WorkerImageSelect({
  ...
  const isMissing = !loading && !error && value !== '' && !tags.includes(value);
  ...
  return (
    <div>
      ...
      >
        ...
        {error && value !== '' && <option value={value}>{value}</option>}
