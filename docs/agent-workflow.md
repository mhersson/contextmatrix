...
for external planning sessions and the web UI, not for card-working agents:
no workflow skill directs an agent at a playbook, and a card-execution
session (`start_workflow`, `execute-task`, `review-task`, ...) has no reason
to call these tools.
