...
- **`sessionlog.Stop` debug-build assertion**
  (`internal/backend/sessionlog/manager.go`). `Stop` documents but does not encode
...
- **`Board.tsx` column virtualisation.** Each column renders every card
  ...
  a perf cliff. Reuse the `VirtualLogList` variable-height pattern from the worker
  console.
...
- **Operator visibility for chat backend container drift.** When `SendChatMessage`
